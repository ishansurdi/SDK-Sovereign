...
from server.policy_runtime import (
    PolicyMap,
    configured_live_modes,
    load_model_agents,
    load_rule_agents,
    load_teacher_agents,
    mode_diagnostics,
...
def _load_agents(mode: str) -> PolicyMap:
    ...
    if mode == "rule":
        agents = load_rule_agents()
    elif mode == "teacher":
        agents = load_teacher_agents()
    else:
        agents = load_model_agents(mode, deterministic=True)
...
def _select_mode(requested_mode: Optional[str]) -> str:
    ...
    available = {item["id"] for item in configured_live_modes()}
...
def register_play_routes(app: Any, env: Any) -> None:
    """Register the HTML demo plus server-side stepping routes."""
    ...
    def play_catalog() -> dict[str, Any]:
        ...
        return {
            ...
            "available_modes": configured_live_modes(),
